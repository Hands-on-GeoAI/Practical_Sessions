-- Jupyter
prepend_path("PATH", "/projappl/project_2018216/GeoAI/bin")

setenv("_COURSE_BASE_NAME", "GeoAI-L4-2")
setenv("_COURSE_NOTEBOOK", "Practical_Sessions/Lecture-4/L4_2-PoolDetection.ipynb")
setenv("_COURSE_GIT_REPO", "https://github.com/Hands-on-GeoAI/Practical_Sessions.git")
setenv("_COURSE_GIT_REF", "")
setenv("_COURSE_NOTEBOOK_TYPE", "notebook")

-- Extra course material
-- setenv("_COURSE_SRC", "/scratch/project_2018216/Databank")
